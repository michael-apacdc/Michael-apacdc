...
- 每天(北京时间上午9点左右)GitHub Actions 自动依次运行三条流水线:
  ...
  4. **AI板块相对强弱模型(观察验证模式)**:对29支AI产业链核心美股(`lib/aiUniverse.ts`)做横截面多因子打分(短期反转/中期动量/残差动量/量能,权重每天从最近约3年数据自动重估),预测未来5个交易日相对等权AI板块的强弱,输出每日排名和前5强势名单。**这个模型发布前做了2022~2026共216个窗口的滚动样本外检验,结论是胜率51.9%、无统计学意义的选股优势 —— 因此它以"观察验证模式"运行**:每天照常输出,5个交易日后自动回填每条信号的真实结果并在页面公示实盘成功率,达标(≥60个信号日、信号日胜率≥58%且平均超额为正)才转正作为正式参考;在那之前不构成买卖依据。点评用 Claude Sonnet。
...
- 网页页面:今日报告(`/`)、历史归档(`/archive`)、亚太选址分析(`/sea`)、量化分析(`/quant`)、美股趋势研判与预警(`/trend`)、AI板块相对强弱(`/portfolio`)。
...
5. 在 Supabase 项目的 SQL Editor 里,依次执行 [`supabase/schema.sql`](supabase/schema.sql)、[`supabase/schema_sea.sql`](supabase/schema_sea.sql)、[`supabase/schema_trend.sql`](supabase/schema_trend.sql)、[`supabase/schema_aiquant.sql`](supabase/schema_aiquant.sql) 的内容,建好全部数据表。
6. **Vercel**(vercel.com)—— 用 GitHub 账号登录,导入这个仓库作为新项目,会自动部署网页。
7. **Tiingo**(tiingo.com,可选,$10/月)—— 比 Yahoo 免费接口更稳定、有官方SLA的行情数据源。注册后在 Account → API 页面拿 token。**不配置也完全可用**(自动用 Yahoo),这是可靠性提升而非收益提升。
...
npm run pipeline:aiquant         # AI板块相对强弱模型,等同于 npx tsx pipeline/runAiQuant.ts
```

运行时会打印每一步的进度,最后会打印本次预估花费。运行完可以去 Supabase 的 Table Editor 里检查对应的表是否有新数据(`daily_reports`/`news_items`/`trend_notes`、`sea_deals`/`sea_country_outlook`、`trend_ticker_signal`/`trend_subsector_snapshot`、`ai_quant_scores`/`ai_quant_picks`/`ai_quant_snapshot`)。
...
- **想增删AI强弱模型跟踪的股票**:改 `lib/aiUniverse.ts` 里的 `AI_UNIVERSE`,推送到 GitHub 即可。注意:池子变了,横截面排名的口径也变了,实盘追踪的历史成功率会混入不同口径的数据,大改后建议清空 `ai_quant_picks` 表重新积累。
- **AI强弱模型什么时候能"转正"**:看 `/portfolio` 页面的"转正进度"卡片。标准:累计≥60个信号日、按信号日聚合的胜率≥58%且平均超额为正。达标后再决定是否作为正式参考,不达标就继续观察或下线。
...
Next.js(网页) + Supabase Postgres(数据库)+ GitHub Actions(每日定时任务)+ Yahoo Finance / Tiingo(股价数据)+ SEC EDGAR(官方财报数据)+ Anthropic Claude API(行业报告用 claude-haiku-4-5,AI强弱模型点评用更强的 claude-sonnet-5),部署在 Vercel。托管/数据库/调度/新闻/股价/财报数据全部免费,持续成本是 Claude API 调用(Haiku 每月几美元到十几美元;AI强弱点评 Sonnet 每天一次、输入量小,预计每月 $1-3)+ 可选的 Tiingo $10/月。
