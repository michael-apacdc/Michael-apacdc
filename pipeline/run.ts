...
async function main() {
  ...
  const counts = await writeReportToDb(reportDate, news, report, status, pipelineMeta);

  console.log(`\n===== 完成 =====`);
  console.log(`报告日期: ${reportDate}`);
  console.log(`状态: ${status}${warnings.length > 0 ? ` (${warnings.join("; ")})` : ""}`);
  console.log(`写入: ${counts.newsWritten} 条新闻, ${counts.notesWritten} 条趋势判断`);
